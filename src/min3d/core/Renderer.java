...
public class Renderer implements GLSurfaceView.Renderer
{
	...
	protected void drawSetup()
	{
		...
		// Custom code. Following code is commented to enable background of jpg. 
...
//		}
	...
	protected void drawScene()
	{
		...
		//Custom Code: Synchronization over scene is necessary here.
		synchronized (_scene) {
...
//		_gl.glRotatef($o.rotation().z, 0,0,1);
		...
		//Custom Code
		if($o.rotation().isCustomRotationEnabled()){
			_gl.glRotatef($o.rotation().initRotAngle, $o.rotation().initXDir, $o.rotation().initYDir, $o.rotation().initZDir);
			_gl.glRotatef($o.rotation().customRotAngle, $o.rotation().xDir, $o.rotation().yDir, $o.rotation().zDir);
		}
		else{
			_gl.glRotatef($o.rotation().x, 1,0,0);
			_gl.glRotatef($o.rotation().y, 0,1,0);
	...
	int uploadTextureAndReturnId(Bitmap $bitmap, boolean $generateMipMap) /*package-private*/
	{
		...
		Log.d(Min3d.TAG, "After binding texture Id");
		...
		Log.d(Min3d.TAG, "After binding 2D image");
		return glTextureId;
	}
	...
	private void reset()
	{
		...
		//Custom Code have to use the same texture manager
